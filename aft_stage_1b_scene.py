# aft_stage_1b_scene.py
import pygame
from constants import GameConstants as GC
from utils import load_image

def run_aft_stage_1b_scene(game_instance):
    print("DEBUG: run_aft_stage_1b_scene - CALLED")
    ...
    npc1_sprite = game_instance.npc1_img_scaled # Asumsi NPC yang sama atau ada atribut lain

    player_rect = pygame.Rect(100, 495, 50, 80) 
    facing_right = True 
    frame_index = 0
    ...
    npc_pos_tuple = (GC.SCREEN_WIDTH - 250, 495 - 10) 
    ...
    dialog_active = False
    dialog_index = 0
    dialog_with_npc_list = game_instance.dialogues.get(GC.STATE_AFT_STAGE_1B, ["NPC: Hebat! Berikutnya adalah awan misterius."]) 
    ...
    try:
        ...
        print("Warning: Font kustom tidak ditemukan di aft_stage_1b_scene, menggunakan default.")
    ...
    print("DEBUG: AFT-Stage 1B Scene - Loop utama dimulai")
    while running:
                    ...
                    if event.key == pygame.K_RETURN:
                        ...
                        if dialog_active:
                            ...
                            if not game_instance.transition.is_active():
                                print("AFT-Stage 1B: Dialog selesai. Memulai transisi ke STATE_AWAN!")
                                # PERBAIKAN DI SINI:
                                game_instance.transition.start(GC.STATE_AWAN, fade_type="out")
        ...
        if player_rect.colliderect(trigger_npc_rect):
            if not dialog_active and not dialog_completed:
                ...
                prompt_message = "Tekan ENTER untuk melanjutkan perjalanan"
        ...
        if transition_initiated_by_this_scene:
            print(f"DEBUG: AFT-Stage 1B Scene - Transisi ke {game_instance.transition.next_state} telah dimulai. Menghentikan loop.")
            running = False

    print(f"DEBUG: AFT-Stage 1B Scene - Loop berakhir. Kembali ke game.py.")
    return